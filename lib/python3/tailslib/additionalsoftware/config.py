...
CONFIG_FILE_OWNER = "tails-persistence-setup"


def _write_config(packages, search_new_persistence=False):
    config_file_owner_uid = pwd.getpwnam(CONFIG_FILE_OWNER).pw_uid
    config_file_owner_gid = grp.getgrnam(CONFIG_FILE_OWNER).gr_gid
    ...
    with atomicwrites.atomic_write(packages_list_path, overwrite=True) as f:
